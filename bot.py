...
async def alive(_, message):
    await message.reply_text("HUKUM KARO AKSHAT BOSS")
...
async def custom_mention(_, message):
    ...
    if not message.reply_to_message:
        return await message.reply_text("BOSS KIS NUNU LESS KI GAND KA KIDA NIKALNA HAI TAG KRO BASS AAP")
...
async def stop_custom(_, message):
    ...
    await message.reply_text(
        "LOL BOSS🤣 USKI TO MAA CHUD GEYA"
    )
